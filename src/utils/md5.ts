...
function binl2b64(binarray: number[]): string {
  ...
  let str: string = ''
  for (let i: number = 0; i < binarray.length * 32; i += 6) {
    str += tab.charAt(
...
function str2binl(str: string): number[] {
  const nblk: number = ((str.length + 8) >> 6) + 1 // number of 16-word blocks
  const blks: number[] = new Array(nblk * 16)
  for (let i: number = 0; i < nblk * 16; i++) blks[i] = 0
...
function strw2binl(str: string): number[] {
  const nblk: number = ((str.length + 4) >> 5) + 1 // number of 16-word blocks
  const blks: number[] = new Array(nblk * 16)
  for (let i: number = 0; i < nblk * 16; i++) blks[i] = 0
  for (let i: number = 0; i < str.length; i++)
